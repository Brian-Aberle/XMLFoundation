...
#include <string.h> // for: strlen()
	...
	#define LONG_ONE 1LL
#endif
//using namespace MemberDescriptor::DataTypes;
//using namespace MemberDescriptor::DataTypes;
#ifndef ASSERT
								...
								const char *pNestedInTag/* = 0*/, ObjectFactory pFactory/* = 0*/ )
{
	...
	if (pList)
	{
		...
		else
		{
			...
			{
				char szTemp[64];
				sprintf(szTemp, "Failed to allocate new Member Descriptor for: %s", pObjectTag);
				TRACE_ERROR(szTemp);
			}
		}
	}
...
void XMLObject::MapAttribute(__int64* pValue, const char* pTag, const char* pzTranslationMapIn, const char* pzTranslationMapOut, int nTranslationFlags)
{
	if (!m_pMappedAttributeHash)
	{
		// Create the hash with a small prime number
...
bool XMLObject::ToXML(GString *xml, int nTabs, const char *TagOverride, unsigned int nSerializeFlags, StackFrameCheck *pStack/* = 0*/, const char *pzSubsetOfObjectByTagName/* = 0*/)
{
	...
	// WRITE_DOCTYPE. It should only appear at the root of the document as we recurse
	if ( (( nSerializeFlags & INCLUDE_DOCTYPE_DECLARATION ) != 0) && Stack.getStackSize() == 1 && nTabs == 0  )
	{
		(*xml) << "<!DOCTYPE ";
	...
	if ( pzSubsetOfObjectByTagName && pzSubsetOfObjectByTagName[0] )
	{
...
void XMLObject::FormatMemberToDump(MemberDescriptor *pMD, GString &strDest, int nTabs, StackFrameCheck *pStack, const char *pzPlace)
{
	...
	else if (pMD->DataType == MemberDescriptor::UserIntegerArray)
	{
		__int64 nIndex = 0;
		int nIsValidIndex = 0;
		__int64 nValue = pMD->m_DataAbstractor.pIntArrayHandler->getAt(pMD->m_Member.pUserIntegerArray,nIndex,&nIsValidIndex);
...
{
	// If Dump() Itself is called on an Object which causes it to crash, this block of code will show you exactly where it was.
//	GString strTypeDestination;
//	GString strN("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\\n");
//	strDest.ToFile("c:\\XMLFoundation\\inProgress.txt");
//	strN << "[" << btRoot->GetTypeName(strTypeDestination) << ": strTagName=" << btRoot->strTagName << "] = ";
//	strN.ToFileAppend("c:\\XMLFoundation\\inProgress.txt");
...
void XMLObject::Dump(GString &strDest, int nTabs/* = 0*/, StackFrameCheck *pStack/* = 0*/)
{
	...
	if (strTypeDescription != "List<XMLObject *>")
	{
		...
		strDest << getOID(); // this will execute the membermappings if necessary to obtain the OID, the code below will not
								/* if (m_pzoid[0])
										{
											strDest << m_pzoid;
										}
...
XMLObject::~XMLObject()
{
	...
	if ( !(m_nBehaviorFlags & PREVENT_AUTO_DESTRUCTION) )
	{
		...
		if (Stack.m_pStack->m_nNext < Stack.m_pStack->m_nSize) // 2024
		{
...
__int64 XMLObject::IncRef(StackFrameCheck *pStack /*=0*/, int nDeep/* = 1*/)
{
	StackFrameCheck Stack(this, pStack);
...
__int64 XMLObject::DecRef(int nUserOwnsObject/* = false*/, StackFrameCheck *pStack /*=0*/)
{
	if (m_nRefCount < 1)
	{
		return m_nRefCount;
	...
	if (pStack && pStack->isNestedStackFrame()) // 2024
	{
		return 0;
	}
	...
	if ( !pStack->isNestedStackFrame() )
	{
		...
		while (pObj)
		{
			...
			if (nChange < 0)
			{
				...
				__int64 lRefCount = pObj->DecRef(nUserOwnsObj, pStack);
				if (lRefCount == 0)
				{
							 ...
							 const char *pTag5, int nSource5)
{
	...
	if (lstOIDKeyParts->Size() == 0)
	{
		...
		__pragma(warning(disable:4312))  // we are intentionally converting int to void *
#endif
...
#ifdef _WIN32
		__pragma(warning(pop)) // disable:4312
#endif
	}
	else
	{
...
int XMLObject::CopyState(XMLObject *pCopyFrom)
{
			...
			else 
			{
				...
				while (pObjSource1)
				{
					...
					if (strVType == "RelationshipWrapper")
					{
						...
						// added 2024 - special case - all other collections return type XMLObject, this is an abstracted string could be std::string, CString, RWString, or GString
						if (pDestMapDataStruct->DataType == MemberDescriptor::UserStringCollection &&  pSrcMapDataStruct->DataType == MemberDescriptor::UserStringCollection) 
						{
							// There are two possible desired behaviors:  
							// 1 - the Source string list entirely replaces the destination string list  (DEFAULT)
							// 2 - items from the Source string list are added to the destination string list
							pDestMapDataStruct->m_DataAbstractor.pStrListHandler->removeAll(pDestMapDataStruct->m_Member.pUserStringCollection); // pDestMapDataStruct->m_Member.pUserStringCollection);

							
							__int64 nSourceStringListCount = pSrcMapDataStruct->GetObjectContainmentCount();
							userStringListIterator iter2 = 0;
							const char* pzEntry = pSrcMapDataStruct->m_DataAbstractor.pStrListHandler->getFirst(pSrcMapDataStruct->m_Member.pUserStringCollection, &iter2);
							while (nSourceStringListCount-- > 0)
							{
								// add pzEntry into the Destination List
								pDestMapDataStruct->m_DataAbstractor.pStrListHandler->append(pDestMapDataStruct->m_Member.pUserStringCollection/*pDestMapDataStruct->m_DataAbstractor.pStrListHandler*/, pzEntry);

								// note: the end of the list is "" as opposed to null - using the pSrcMapDataStruct->GetObjectContainmentCount() to determine how many iterations to make
								if (nSourceStringListCount)
									pzEntry = pSrcMapDataStruct->m_DataAbstractor.pStrListHandler->getNext(pSrcMapDataStruct->m_Member.pUserStringCollection, &iter2);
							}
						}
						else // if (pSrcMapDataStruct) // note pSrcMapDataStruct is always non null
						{
	...
	bool bFactoryOrigin = (GetObjectDescriptor()) ? true : false;
	if (bFactoryOrigin)
	{
		// this object was created by the ObjectFactory() logic underneath FromXML()
	}
	else
	{
		// this object which is derived from XMLObject was instanciated by application code
		// for example  CustomerObject pC = new CustomerObject;   
		// where the members were assigned by the application code: for example pC->name="Root",  not by the ObjectFactory via MapMember()
	}
	
	// This was the previous code - without the unnecessary temp variable:
//	return SetObjectDescriptor(pCopyFrom->GetObjectDescriptor());

//  if the [pCopyFrom] object was factory created, keep creator's map. 
	int nSetODRetVal = SetObjectDescriptor(pCopyFrom->GetObjectDescriptor());
	if (nSetODRetVal)
		return 1;		// soft break here
	return 0;			// and here            to study the object state during the condition differences
}
...
__int64 XMLObject::GetObjectDescriptorCount()
{
...
{ 
	...
	// This is the first instance of this <Type> if XMLObjects are only instanciated by FromXML() 
	return 1; 
}
