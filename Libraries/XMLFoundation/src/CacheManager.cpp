...
// findObject() returns an object from the cache or null if the object was not found in the cache.
// hashKey is the string concatination of the OID and C++ class name that was derived from XMLObject
...
int XMLObjectCache::setMappedValue(const char* pzOIDClassName, const char* pzTagName, const char* pzValue)
{
	if (!pzOIDClassName || !pzTagName || !pzValue)
		return 0; // argument error - object not found

	XMLObject* pObject = (XMLObject*)m_cache->Lookup(pzOIDClassName);
	if (pObject)
	{
		pObject->SetMemberByTag( pzTagName, pzValue );
		return 1; // success or [pzTagName] was not mapped to this object - so there was nothing to do.  top see bad [pzTagName] call SetLogFile("c:\\XMLFoundation\\MyAppDebugLog.txt");
	}
	return 0; // object not found 
}

int XMLObjectCache::getMappedValue(const char* pzOIDClassName, const char* pzTagName, GString* strValueDest, int nSource)
{
	if (!pzOIDClassName || !pzTagName || !strValueDest)
		return 0; // argument error - object not found
	XMLObject* pObject = (XMLObject*)m_cache->Lookup(pzOIDClassName);
	if (pObject)
	{
		if (nSource == 1) // element
		{
			pObject->GetMemberByTag(pzTagName, *strValueDest);
		}
		else if (nSource == 0) // attribute
		{
			pObject->FindAttribute(pzTagName, *strValueDest);
		}
		else 
		{
			return 0; // invalid [nSource] value
		}
		return 1; // success
	}
	return 0; // object not found or pzTagName was not mapped, the globally calling  SetLogFile("c:\\XMLFoundation\\MyAppDebugLog.txt"); shows if [pzOIDClassName] was found and [pzTagName] was invalid
}
...
void XMLObjectCache::enterObject(const char *oid, const char *pzClassName, XMLObject *pObject )
{
	if ( !(pObject->GetObjectBehaviorFlags() & PREVENT_AUTO_CACHE) )
	{
		...
		XML_LOCK_MUTEX(&m_cs);    //	gthread_mutex_lock(&m_cs);    or a CRITICAL_SECTION

		m_cache->Insert((const char *)strKey, pObject);

		XML_UNLOCK_MUTEX(&m_cs);  // gthread_mutex_unlock(&m_cs); 
...
XMLObject *XMLObjectCache::getObject(const char *pzXMLTag, const char* oid, const char *pzUpdateTime,int *bDidCreate /*= 0*/ )
{
...
// ****************************************************************
void XMLObjectCache::EnterCacheData(__int64 length, void *pData, const char *pzKey)
{
...
void *XMLObjectCache::GetCacheData(const char *pzKey, __int64 *length)
{
	void *pRet;
	gthread_mutex_lock(&m_csData);
	...
	if (pRet && length)
	{
		...
		*length = (__int64)m_cacheData->Lookup((const char *)strKey);
	}
	gthread_mutex_unlock(&m_csData);
	return pRet;
}
