...
bool CWinINetHTTPDataSource::internetConnect(const char *pszServer/* = 0*/, int nPort /* = 80 */)
{
	...
	m_hConnection = InternetConnectA(
						...
						(INTERNET_PORT)nPort,						// 443 if SSL, usually 80 otherwise
						NULL,						/* user name */
						NULL,						/* user password */ 
...
RE_POST:
	...
	try
	{
		...
		if (!pzHTMLDoc)
		{
			...
			buffer.dwBufferTotal = (DWORD)(nCount + nXMLLen);
RE_SEND1:
		...
		else
		{
			if (nXMLLen && xml)
			{
				buffer.dwBufferTotal = (DWORD)nXMLLen;
				if (!HttpSendRequestEx(m_hFile, &buffer, NULL, HSR_INITIATE, 1))
					ThrowLastError();
			}
			else
...
RE_SEND:
		...
		if (nXMLLen && xml)
		{
			if (!InternetWriteFile(m_hFile, xml, (DWORD)nXMLLen, &dwBytes))
				ThrowLastError();
