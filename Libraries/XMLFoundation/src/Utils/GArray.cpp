...
#include <stdio.h> // for: strlen(), strcpy(), strcmp()
...
{
	__int64 nKeyLen = strlen(szKey) + 1;
	...
	if (!pCurrentKeyBlock)
	{
		m_nCurrentKeySpaceSize = m_nInitialKeySpaceSize;
	...
	else
	{
		...
		m_nCurrentKeySpaceSize =  m_nKeySpaceGrowBy;
		pCurrentKeyBlock = new char[m_nCurrentKeySpaceSize];
		m_KeySpace.Push(pCurrentKeyBlock);
...
__int64 GArray::AddElement(void *Element, const char *szKey /* = 0 */)
{
	...
	if (szKey)
	{
		char *p = MakeKey(szKey); 
		(*(m_arrObjects + m_nNext)).m_pszKey = p;
	}
...
void GArray::Assign(__int64 nIdx, void *pValue, const char *szKey /* = 0 */)
{
	...
	{
		char* p = MakeKey(szKey);
		(*(m_arrObjects + nIdx)).m_pszKey = p;
	}
	else
	{
		(*(m_arrObjects + nIdx)).m_pszKey = 0;
	}
...
void GArray::Initialize()
{
	...
	m_nCurrentKeySpaceIndex = 0;
	m_nCurrentKeySpaceSize = 0;
	m_nInitialKeySpaceSize = 8192;
	m_nKeySpaceGrowBy = 512;
