...
// This GThread source is based on a customized version of pthreads that was used by the XMLFoundation from 2000 until July 2007.
...
// This entire .cpp file should only be compiled in Windows builds 
#if defined(_WIN32) || defined(_WIN64) || defined(__WINPHONE) 
...
#ifdef ___ANDROID
    #include <time.h> // to fill local _ftime structures
#else  // ___LINUX , ___LINUX64, _WIN64, _WIN32
  ...
  if ((lock_result = gthread_mutex_lock (&(cv->waitersLock))) == 0)
    {
      ...
      ///////////////////////////////////////////////////////////////////
      //      lastWaiter = cv->wasBroadcast && (cv->waiters == 0);   <--- original code is correct
      //
      // if [wasBroadcast] == 1 and we just decremented [waiters] down to 0 then [lastWaiter] is set to 1 
      // otherwise set [lastWaiter] to 0
      //
      // the new code is more readable and does not cause a compiler warning
      if (cv->wasBroadcast && !cv->waiters)
      {
          lastWaiter = 1;
      }
      ///////////////////////////////////////////////////////////////////////////////////////
      ...
      */
{
  ...
  if ((self = (gthread_t) gthread_getspecific (_gthread_selfThreadKey)) 
    ...
    status = WaitForMultipleObjectsEx( nHandles, handles, FALSE,  timeout,  false  );
    ...
    {
      /*
       Either got the mutex or the cancel event was signaled
       */
          ...
          if (self != NULL && !self->implicit)
            {
              ...
              exceptionInformation[0] = (ULONG_PTR)(_GTHREAD_EPS_CANCEL);
...
static DWORD ExceptionFilter (EXCEPTION_POINTERS * ep, __int64 * ei)
{
  DWORD param;
...
//void *_gthread_threadStart(void* p)
...
//  DWORD exceptionInformation[3];
...
#if ! defined (__MINGW32__) || defined (__MSVCRT__)
	#ifndef __WINPHONE
	   // In all cases within XMLFoundation, gthread_exit() returns 0 so this 
       // safe data truncation for any integer return code less that 4.2 Billion
      __pragma(warning(suppress:4302))
      __pragma(warning(suppress:4311))
  ...
  exceptionInformation[0] = (ULONG_PTR) (_GTHREAD_EPS_EXIT);
  exceptionInformation[1] = (ULONG_PTR) (value_ptr);
  exceptionInformation[2] = (ULONG_PTR) (0);

  RaiseException (
		  EXCEPTION_GTHREAD_SERVICES,  //dwExceptionCode
		  0,//dwExceptionFlags
		  3,//nNumberOfArguments
		  EXCEPTIONCAST exceptionInformation);
