...
void GProfile::ProfileParse(const char *szBuffer, __int64 dwSize)
{
	...
	while (nIdx < dwSize)
	{
		...
		if (strLine.GetAt(0) == '[')
		{
			__int64 nIdx2 = strLine.Find(']');
			if (nIdx2 == -1)
				nIdx2 = strLine.Length();

			// new section
			pSection = new GProfileSection;
			pSection->m_strName = strLine.Mid(1, nIdx2 - 1);
		...
		{
			__int64 nIdx3 = strLine.Find('=');
			if (nIdx3 == -1)
				continue;
			GProfileEntry *pNVP = new GProfileEntry;
			pSection->m_lstNVP.AddLast(pNVP);
			pNVP->m_strName = strLine.Left(nIdx3);
			pNVP->m_strName.TrimLeftWS();
			pNVP->m_strName.TrimRightWS();
			
			pNVP->m_strValue = strLine.Mid(nIdx3 + 1);
...
__int64 GProfile::WriteCurrentConfigSection(GString *pzDestStr, const char *pzSection, bool bWriteXML/* = 0*/)
{
	return GProfile::WriteCurrentConfigHelper(0,pzDestStr, pzSection, bWriteXML);
}


// Serialize memory config
// Returns the number of bytes written to the destination on success or 0 for failure.
__int64 GProfile::WriteCurrentConfigHelper(const char *pzPathAndFileName, GString *pDest, const char *pzSection/*=0*/, bool bWriteXML/*=0*/)
{
...
__int64 GProfile::WriteCurrentConfig(const char *pzPathAndFileName, bool bWriteXML/* = 0*/)
{
	return GProfile::WriteCurrentConfigHelper(pzPathAndFileName,0,0,bWriteXML);
}
__int64 GProfile::WriteCurrentConfig(GString *pzDestStr, bool bWriteXML/* = 0*/)
{
...
int GProfile::ChangeNotify(const char *pzSection, const char *pzEntry, const char *pzValue)
{
	...
	else 
	{
		...
		GString strKey2(pzSection);
		strKey2.MakeUpper();
		fn  = (fnChangeNotify)m_pTreeNotify->search(strKey2);
		if (fn)
		{
			return fn(pzSection, pzEntry, pzValue);
		}
