...
#include "GString.h"
//#include "GString0.h"  // removed 2024
//#include "GString32.h" // removed 2024
...
#include <string.h> // for: memcpy(), strlen(), strcmp(), memmove(), strchr(), strstr(), strncmp()
...
XMLResultset *g_pEmptyXMLResultSet;
GlobalInit::GlobalInit()
{
	g_pEmptyXMLResultSet = new XMLResultset();
}
GlobalInit::~GlobalInit()
{
	if (g_pEmptyXMLResultSet)
		delete g_pEmptyXMLResultSet;
}
...
GString & GString::operator=(const wchar_t * _p)
{
	...
	while (_p && *_p)
	{
		if (_len >= _max)
			resize();
		_str[_len] = (char)*_p;  // conversion from a 16bit char to an 8 bit char - this is not for unicode - the conversion is only for ASCII
		_p++;
		_len++;
	}
...
{
	char  szBuffer[25];
	snprintf(szBuffer, sizeof(szBuffer), "%hi", _p);
...
{
	char  szBuffer[25];
	snprintf(szBuffer, sizeof(szBuffer),"%hu", _p);
...
{
	char  szBuffer[25];
	snprintf(szBuffer, sizeof(szBuffer), "%i", _p);
...
{
	char  szBuffer[25];
	snprintf(szBuffer, sizeof(szBuffer), "%u", _p);
...
{
	char  szBuffer[25];
	snprintf(szBuffer, sizeof(szBuffer), "%li", _p);
...
{
	char  szBuffer[25];
	snprintf(szBuffer, sizeof(szBuffer), "%lu", _p);
...
{
	char  szBuffer[50];
	snprintf(szBuffer, sizeof(szBuffer), g_FloatFmt._str, _p);
	__int64 nLen = strlen(szBuffer);
...
{
	char  szBuffer[50];
	snprintf(szBuffer, sizeof(szBuffer), "%.6g", _p);
	__int64 nLen = strlen(szBuffer);
...
{
	char  szBuffer[50];
	snprintf(szBuffer, sizeof(szBuffer), "%.6Lg", _p);
	__int64 nLen = strlen(szBuffer);
...
{
	char  szBuffer[25];
	snprintf(szBuffer, sizeof(szBuffer), "%ld", _p);
...
GString & GString::operator+=(double _p)
{
	...
	char  szBuffer[1080];
	snprintf(szBuffer, sizeof(szBuffer), "%lf", _p);
...
GString GString::FindStringBetween(const char *pSearchForBegin, const char *pSearchForEnd) const
{
	...
	if (nIndex > -1)
	{
		size_t nSearchBeginLen = strlen(pSearchForBegin);
...
void GString::ReplaceChars(const char *pzAnyOfThese, char chBecomesThis)
{
	char *p = strpbrk(_str, pzAnyOfThese);
	while(p)
	{
		*p = chBecomesThis;
		p = strpbrk(p, pzAnyOfThese);
	}
}
			...
			case 'f':
				{
					...
					snprintf(szBuffer, sizeof(szBuffer), "%*.*f", nWidth, nPrecision+6, f );
					nItemLen = strlen(szBuffer);
...
const char *GString::EscapeWithHex(const char *pzEscapeChars)
{
	...
	for(__int64 i=0;i < nSourceBytes; i++)
	{
		...
		{
			char buf[20];
			snprintf(buf, sizeof(buf), "%%%02X", pSrc[i]);
			strDestrination += buf;
		}
		else 
		{
...
const char *GString::UUEncode()
{
	BUFFER b;
	BufferInit(&b);
	uuencode((unsigned char *)_str, (int)_len, &b); 
...
// in openssl3.0 the available options are:
/*
RSA-SHA384
ripemd160WithRSA
sm3
...
/*
void HashKeyAlgorithmsCallback(const OBJ_NAME* obj, void* pGStrList)
{
	GStringList* pList = (GStringList*)pGStrList;
	pList->AddLast(obj->name);
}

void GString::HashKeyAlgorithms(GStringList* pListDest, GString* pStrDest)
{
#ifndef __NO_OPENSSL
	GStringList* pResults = 0;
	if (!pListDest)
		pResults = new GStringList();
	else
		pResults = pListDest;

	OpenSSL_add_all_digests();
	OBJ_NAME_do_all(OBJ_NAME_TYPE_MD_METH, HashKeyAlgorithmsCallback, (void*)pResults);
	if (pStrDest)
		*pStrDest = pResults->Serialize("\n", 0);
	if (!pListDest)
		delete pResults;
#endif
}
...
#ifndef __NO_OPENSSL
bool OpenSSLHashDigest(unsigned char* pzData, __int64 nDataLength, unsigned char* pzDigestDest, const char *pzDigestName)
{
	EVP_MD_CTX* mdctx;
	const EVP_MD* md;
	...
	OpenSSL_add_all_digests();
	md = EVP_get_digestbyname(pzDigestName);
	if (!md)
	{
		return false;
		// fail
	...
	EVP_DigestUpdate(mdctx, pzData, nDataLength);
...
bool GString::HashKey(const char *pzHashName, GString* pHashDest)
{
	if (!pHashDest)
		return 0;  // fail - null pHashDest
	GString strHashName(pzHashName);

	if (strHashName.CompareNoCase("RotatingHash") == 0)
	{
		...
		memcpy(pHashDest->_str, &h, 4);
		pHashDest->SetLength(4); // 4 bytes is 32 bits
		return 1;// success
	}
	else 
	{
		//return OpenSSLHashDigest((unsigned char*)_str, _len, (unsigned char*)pHashDest->_str, pzHashName);
	}
...
bool GString::Compress( GString *pSrc )
{
	...
	SetPreAlloc( (__int64)(nZippedLength + (nZippedLength * .01) + 600) );
...
//
#if defined (___LINUX) 
#ifndef ___ANDROID  // iconv() has been ported to Android, it's just not part of Android and has not been needed yet by apps driving XMLFoundation
...
void GString::FromUnicode(const wchar_t *wSrc, __int64 nTerminateCount)
{
	...
	int nBytes = WideCharToMultiByte(CP_ACP, 0, wSrc, (int)nLen, NULL, NULL, NULL, NULL);
	SetPreAlloc(nBytes + 1);
	WideCharToMultiByte(CP_ACP, 0, wSrc, (int)nLen, _str, nBytes, NULL, NULL);
	SetLength(nBytes);
#else
	#if defined(___ANDROID) || defined (___IOS)
...
wchar_t *GString::Unicode()
{
    ...
    int len = MultiByteToWideChar(CP_ACP, 0, _str, (int)_len, NULL, 0) ;
    int nAllocLen = (sizeof(unsigned short) * len);
	_pWideStr = (wchar_t *)malloc(nAllocLen + 2); 
	memset(_pWideStr,0,nAllocLen+2);
	MultiByteToWideChar(CP_ACP, 0, _str, (int)_len, (LPWSTR)_pWideStr, nAllocLen);
...
GString::operator const wchar_t * ()
{
    if (_pWideStr)
	{
		...
		_pWideStr = 0; // note: the typecast is necessary to cast off the constness of _pWideStr so that it may be assigned a value from within a const method
	}
...
{
	__int64 a = Gstrlen(unicodestr)+1;
	char *_p = new char[a];
	::WideCharToMultiByte( CP_ACP, 0, unicodestr, -1, _p,(int)a, NULL, NULL);
