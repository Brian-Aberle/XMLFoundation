...
{ 
	...
	GString strDriverPath = GetProfile().GetPath("System", "LanguageDrivers");
	GString strErrorDetail;
...
strErrorDetail << "Attempt:" << strPathedDynamicLib << "\n";
		m_dllHandle = _OPENLIB( strPathedDynamicLib );
		if (m_dllHandle)
		{
strErrorDetail << "Attempt:Succeeded\n";
			// dlopen succeeded for [strPathedDynamicLib]

			try // No Destructor cleanup when we throw from the Constructor
			{
				// if the "GetDriverName" entry point is found
				pfn = (void *)_PROCADDRESS(m_dllHandle, "GetDriverName");
				if (pfn)
				{
strErrorDetail << "Found:Entry Point GetDriverName\n";
				...
				{
strErrorDetail << "NOT Found:Entry Point GetDriverName\n";
				}
			}
			catch(...)
			{
		...
		{
			// dlopen failed for [strPathedDynamicLib]
strErrorDetail << "dlopen() failed\n";
