										  ...
										  unsigned int nParentBehaviorFlags)
{
	...
	while (tok->m_type != xml::_endTag && tok->m_type != xml::_emptyEndTag)
	{
		...
		else if ( tok->m_type == xml::_startTag )
		{
			...
			MemberDescriptor *pMapStartTag = (pObjCurrent) ? pObjCurrent->GetEntry( tok->get() ) : 0;
			// Note: [pMapStartTag] is scoped to this _startTag

			if ( !pMapStartTag)
			{
				if (m_pProtocolObject)
				{
					pMapStartTag = m_pProtocolObject->GetEntry( tok->get() );
				}
				if (!pMapStartTag)
				{
					pMapStartTag = HandleExceptions( tok->get(),pObjCurrent);
				}
				if ( !pMapStartTag && pObjCurrent )
				{
					pMapStartTag = pObjCurrent->HandleUnmappedMember( tok->get() );
				}
				if (!pMapStartTag)
				{
...
//			unsigned int nDeepFlags = 0;			// this was the original code (for years...)
			...
			if (pMapStartTag && pMapStartTag->IsSubObject())
			{
				pSubObject = GetCoorespondingSubObject(pMapStartTag,tok,&nDeepFlags);
			}
			else
			{
				pSubObject = 0;
...
//  {int i; i++;}
 				...
 				receiveIntoObject(pSubObject, pMapStartTag, nDeepFlags, nObjBehaviorFlags);
				if ( pSubObject && nObjBehaviorFlags & SUBOBJECT_UPDATE_NOTIFY )
				{
					// send 'this' a notification that a sub object has been updated
					pObjCurrent->ObjectMessage(MSG_SUBOBJECT_UPDATE, pMapStartTag->strTagName, 0, 0, pSubObject);
				}
			}
...
void XMLObjectFactory::FactoryInitComplete(XMLObject *pO, unsigned int nDataFlags, unsigned int nObjBehaviorFlags)
{
	...
	// NOTE: an "Early Bound OID" creates no temp object and requires no copy because
	// the runtime object instance becomes direct storage for the xml tokenizer that 
	// opbtained the OID as the first attribute and used that key to obtain the cached instance.
...
void XMLObjectFactory::extractObjects( XMLObject *pRootObject,  XMLObject *pSecondaryMapHandler /* = 0*/)
{
	...
	if (m_pException)
	{
		...
		{
			// This distinguishes Client errors from Server errors - caught identically in the application layer
			// [m_pzExceptionThrower] is generally the remote who put an Exception inside a resultset - "Database offline" or something prohibited the proper result generation
