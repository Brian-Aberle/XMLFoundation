...
CXMLNode::CXMLNode(CXMLElementx *element) :
	m_bOwnsMemory(0),
	m_node(0),
	m_bDisableOutputEscaping(0)
{
...
// 
//*********************************************************
CXMLAttribute *CXMLElementx::addAttribute(const char *tag, const char *value,
...
CXMLAttribute *CXMLElementx::addAttribute(const char *tag, __int64 nTagLen,
...
CXMLElementx *CXMLElementx::addChild(const char *tag, 
								   const char *value)
{
	return addChild(new CXMLElementx(tag,value,this));
}

// this data will only be pointed to by the CXMLElementx so it 
// must be valid for the duration of this element.
CXMLElementx *CXMLElementx::addChild(const char *tag, __int64 nTagLen, 
								   const char *value, __int64 nValueLen)
{
	return addChild(new CXMLElementx(tag, nTagLen, value, nValueLen, this));
}
...
CXMLElementx *CXMLElementx::addChild(const char *tag, __int64 nTagLen)
{
	return addChild(new CXMLElementx(tag, nTagLen, this));
}
...
// 
//*********************************************************
CXMLElementx *CXMLElementx::addChild(CXMLElementx *pElement, CXMLElementx *ia /* = 0 */)
{
	if (!m_pChildrenlist)
	{
...
// 
//*********************************************************
CXMLElementx::CXMLElementx(const char *tag, 
						 const char *value,
						 CXMLElementx *parent) :
	m_bOwnsTag(true)
{
...
CXMLElementx::CXMLElementx(const char *tag, __int64 nTagLen, 
						 const char *value, __int64 nValueLen,
						 CXMLElementx *parent)  :
	m_bOwnsTag(false)
{
...
CXMLElementx::~CXMLElementx()
{
	...
	if (m_pChildrenlist)
	{
		...
		while (itChildren())
		{
			delete (CXMLElementx *)itChildren++;
		}
...
void CXMLElementx::createXML(GString& str_xml, 
							int nTabs/* = 0*/, 
		...
		while (it())
		{
			...
			else
			{
				if (bNewLine && bBeautify)
					str_xml << "\n";
				((CXMLElementx *)p->getValue())->createXML(str_xml, nTabs + 1, false, bBeautify, bShortHand);
				bNewLine = false;
			}
		}
...
void CXMLElementx::removeAllChildren()
{
	//for each item in the child list:
	if (m_pChildrenlist)
	{
...
void CXMLElementx::removeChild(CXMLElementx *pElement, bool bDestroy /* = true */)
{
	...
	if (m_pChildrenlist)
	{
		...
		while (m_pChildrenlist->Current() != NULL)
		{
			...
			{
				if (bDestroy)
					delete (CXMLElementx *)m_pChildrenlist->Current();
...
void CXMLElementx::setParent(CXMLElementx *np, CXMLElementx *ia /* = 0 */)
{
	CXMLElementx *parent = getParent();
	if (parent)
...
CXMLAttribute *CXMLElementx::findAttribute(const char *tag) const
{
	if (m_pAttributelist)
	{
		...
		while (itAtts())
		{
			...
			{
				if (_strnicmp(p->getTag(), tag, p->getTagLen() ) == 0)
				{
					return p;
				}
			}
...
CXMLElementx *CXMLElementx::findChild(const char *tag, int nOccurrence/* = 1*/) const
{
	...
	if (m_pChildrenlist)
	{
		...
		while (itChildren())
		{
			CXMLElementx *p = (CXMLElementx *)itChildren++;
			if (p->getTagLen() == (int)strlen(tag))
			{
				if (_strnicmp(p->getTag(), tag, p->getTagLen() ) == 0)
				{
...
//void CXMLElementx::toFile(const char *dst, long nGrowBy /* = 5000 */) const
//{
