...
//
// From 2000 up until 2014 GProfile used only the .INI file format which is VERY easy to hand edit,
...
protected:
	...
	__int64 WriteCurrentConfigHelper(const char *pzPathAndFileName, GString *pDest, const char *pzSection = 0, bool bWriteXML = 0);
...
public:
	...
	// An existing file will be overwritten, GString as [pzDestStr] will be appended to.
	// Returns the number of bytes written to the destination on success or 0 for failure.
	__int64 WriteCurrentConfig(const char *pzPathAndFileName, bool bWriteXML = 0);
	__int64 WriteCurrentConfig(GString *pzDestStr, bool bWriteXML = 0);
	// just like WriteCurrentConfig(), but only serialize a single [section]
	__int64 WriteCurrentConfigSection(GString *pzDestStr, const char *pzSection, bool bWriteXML = 0);
