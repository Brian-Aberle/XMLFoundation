...
// --------------------------------------------------------------------------
#ifndef _G_STRING32_HH_
#define _G_STRING32_HH_
...
// from GString to GString32 that is nearly what a C++ compiler does for a template class
...
protected:
	...
	// When a GString32 < DEFAULT_INITIAL_SIZE bytes is constructed, _initialbuf is used.
	...
	char _initialbuf[GSTRING_INITIAL_SIZE_32]; // do not access this variable, use _str
...
public:
	...
	GString32(__int64 nInitialSize = GSTRING_INITIAL_SIZE_32, int nGrowByAllocationSize = -1);

	// constructs a copy of the source string 
	GString32(const GString &src);
	GString32(const GString &src, __int64 nSourceCount);

	// constructs a copy of the character string
	GString32(const char *pzSource);
	GString32(const char *pzSource, __int64 nSourceCount);

	// constructs a string with ch repeated nCnt times
	// if you get a compile error "2 overloads have similar conversions"
	// while using this ctor cast nCnt to a short:   GString32 strXs('X',(short)777)
	GString32(char ch, short nTimesToRepeat);

	// This will create a GString32 from a double byte BSTR, or Platform::String^ in .NET managed code like this:
	// Platform::String^ MyString = "MyData";
	// new GString32(MyString->Data());
	// wchar_t is a native type in VS2012. It is not in VC6 where <stdio.h> defines it as unsigned short.
	GString32(const wchar_t *strWide);
	

	// Attach to a memory buffer, WITHOUT making a copy of the memory.
	// ------------------------------------------------------------
	// constructs a string attached to supplied memory
	// so that the memory is NOT copied into this GString32.
	// See notes for Attach(), they apply to this ctor as well
	// so to create a GString32 on the heap that uses memory on the stack:
	//
	//     char p[512]; strcpy(p,"hello world");
	//     GString32 *p2 = new GString32(p,strlen("hello world"),512,0);
	//
	...
	GString32(const char *pzSource, __int64 nSourceBytes, __int64 nSourceMemorySize, int nOwn);
	...
	//
	// nOwn is true(1) or false(0) indicating if the GString32 object now owns the memory.
	// if nOwn == true, then you alloc(new) pzSource, and the GString32 will free(delete) it
	...
	// buf is NOT deleted by this object because you indicated GString32 does not own it.
	//
	// If any append operation(like << or +=) extends past the region of memory YOU allocated 
	// that is nSourceMemorySize bytes  - GString32 will allocate new memory and the attached 
	// memory will be deleted if nOwn = true otherwise (if nOwn = false) the buffer will be 
	// unattached from the GString32 and left for you to delete.
	//
	...
	// GString32 never tries to strlen(pzSource) to determine the size.
	...
	// if src is not null terminated it will be terminated in the GString32 owned copy of it.
	// 
	...
	// write() appends to the GString32 to mimic ostream 
	//
	...
	// s.Length() > strlen(s);
	void write(const char *pSource,__int64 nBytes); // lower case for ostream interoperability just replace "ostream" with GString32 types and recompile for 2X string manipulation speed.
	...
	// Load this GString32 from a file and replace any contents in this GString32 - returns 1 for success, 0 for fail
	bool FromFile(const char* pzFileName, bool bThrowOnFail = 1);
	// Append the contents of the specified file to the end of this GString32 - returns 1 for success, 0 for fail
	bool FromFileAppend(const char* pzFileName, bool bThrowOnFail = 1);
	// Save this GString32 to a file - overwrite if file exists
	bool ToFile(const char* pzFileName, bool bThrowOnFail = 1);
	// Append this GString32 to a file - create file if not existing  returns 1 for success, 0 for failure
	...
	// FormatNumber formats the value of the GString32 using the format
	...
	GString32 & operator=(char);
	GString32 & operator=(unsigned __int64);
	GString32 & operator=(__int64);
	GString32 & operator=(const char *);
	GString32 & operator=(const signed char *);
	GString32 & operator=(unsigned char);
	GString32 & operator=(signed char);
	GString32 & operator=(short);
	GString32 & operator=(unsigned short);
	GString32 & operator=(int);
	GString32 & operator=(unsigned int);
	GString32 & operator=(long);
	GString32 & operator=(unsigned long);
	GString32 & operator=(float);
	GString32 & operator=(double);
	GString32 & operator=(long double);
	GString32 & operator=(const GString &);
	GString32 & operator=(const GString32 &);

	// append operators, enables a GString32 to mimic an ostream for simple porting to faster GString32s
	GString32 & operator<<(unsigned __int64);
	GString32 & operator<<(__int64);
	GString32 & operator<<(const char *);
	GString32 & operator<<(const signed char *);
	...
	friend GString operator+(GString32 &, GString32 &);
	friend GString operator+(GString32 &, const char *);
	friend GString operator+(const char *, GString32 &);
	friend GString operator+(GString32 &, const signed char *);
	friend GString operator+(const signed char *, GString32 &);
	friend GString operator+(GString32 &, char);
	friend GString operator+(char, GString32 &);
	friend GString operator+(GString32 &, unsigned char);
	friend GString operator+(unsigned char, GString32 &);
	friend GString operator+(GString32 &, signed char);
	friend GString operator+(signed char, GString32 &);
...
#ifdef _UNICODE
	GString32 & operator<<(const wchar_t *);
	GString32 & operator+=(const wchar_t *);
