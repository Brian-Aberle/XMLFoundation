...
{
public:
	// by default this value is 1 causing all GLists application wide to use a global instance of GListNodeCache
	// This is new in 2024 providing a great performance improvement with a potential to cause an application to use too much memory
	// If the Node caching is causing you problems set GList::bEnableGlobalNodeCache = 0 in your application initialization or startup
	static int bEnableGlobalNodeCache;
...
protected:
	...
	GListNodeCache *pNodeCache; // Every GList has its own pNodeCache
	int bOwnsCache;
public:
	GListNodeCache *GetNodeCache(){return pNodeCache;}
	void SetNodeCache(GListNodeCache *p) {pNodeCache = p;}
	void EnableCache(bool bAutoGarbageCollect = 0, bool bUseGlobalCache = 0);
	...
	__int64 GetCount(void) const { return iSize;}; 
