...
// --------------------------------------------------------------------------
#ifndef _G_STRING0_HH_
#define _G_STRING0_HH_


#include "GString.h"
class GString32;

#define GSTRING_INITIAL_SIZE_0		1  // set to 1 so that we always have room for a 0 byte, null terminated string


// This class hand implements a "template" kind of GString, it could be a maintainance problem
// however GString is mature enough that there will be few fixes and enhancements.  Those fixes
// and enhancements must be hand copied from GString.cpp into this file.  I'd been thinking of
// this for years and waiting until GString was almost "done", before this grand scale cut-and-paste
// from GString to GString0 that is nearly what a C++ compiler does for a template class
...
protected:
	...
	// When a GString0 < DEFAULT_INITIAL_SIZE bytes is constructed, _initialbuf is used.
	...
	char _initialbuf[GSTRING_INITIAL_SIZE_0]; // do not access this variable, use _str
...
public:
	...
	GString0(__int64 nInitialSize = GSTRING_INITIAL_SIZE_0, int nGrowByAllocationSize = -1);

	// constructs a copy of the source string 
	GString0(const GString &src);
	GString0(const GString &src, __int64 nSourceCount);

	// constructs a copy of the character string
	GString0(const char *pzSource);
	GString0(const char *pzSource, __int64 nSourceCount);

	// constructs a string with ch repeated nCnt times
	// if you get a compile error "2 overloads have similar conversions"
	// while using this ctor cast nCnt to a short:   GString0 strXs('X',(short)777)
	GString0(char ch, short nTimesToRepeat);

	// This will create a GString0 from a double byte BSTR, or Platform::String^ in .NET managed code like this:
	// Platform::String^ MyString = "MyData";
	// new GString0(MyString->Data());
	// wchar_t is a native type in VS2012. It is not in VC6 where <stdio.h> defines it as unsigned short.
	GString0(const wchar_t *strWide);
	

	// Attach to a memory buffer, WITHOUT making a copy of the memory.
	// ------------------------------------------------------------
	// constructs a string attached to supplied memory
	// so that the memory is NOT copied into this GString0.
	// See notes for Attach(), they apply to this ctor as well
	// so to create a GString0 on the heap that uses memory on the stack:
	//
	//     char p[512]; strcpy(p,"hello world");
	//     GString0 *p2 = new GString0(p,strlen("hello world"),512,0);
	//
	...
	GString0(const char *pzSource, __int64 nSourceBytes, __int64 nSourceMemorySize, int nOwn);
	...
	//
	// nOwn is true(1) or false(0) indicating if the GString0 object now owns the memory.
	// if nOwn == true, then you alloc(new) pzSource, and the GString0 will free(delete) it
	...
	// buf is NOT deleted by this object because you indicated GString0 does not own it.
	//
	// If any append operation(like << or +=) extends past the region of memory YOU allocated 
	// that is nSourceMemorySize bytes  - GString0 will allocate new memory and the attached 
	// memory will be deleted if nOwn = true otherwise (if nOwn = false) the buffer will be 
	// unattached from the GString0 and left for you to delete.
	//
	...
	// GString0 never tries to strlen(pzSource) to determine the size.
	...
	// if src is not null terminated it will be terminated in the GString0 owned copy of it.
	// 
	...
	// write() appends to the GString0 to mimic ostream 
	//
	...
	// s.Length() > strlen(s);
	void write(const char *pSource,__int64 nBytes); // lower case for ostream interoperability just replace "ostream" with GString0 types and recompile for 2X string manipulation speed.
	...
	// Load this GString0 from a file and replace any contents in this GString0 - returns 1 for success, 0 for fail
	bool FromFile(const char* pzFileName, bool bThrowOnFail = 1);
	// Append the contents of the specified file to the end of this GString0 - returns 1 for success, 0 for fail
	bool FromFileAppend(const char* pzFileName, bool bThrowOnFail = 1);
	// Save this GString0 to a file - overwrite if file exists
	bool ToFile(const char* pzFileName, bool bThrowOnFail = 1);
	// Append this GString0 to a file - create file if not existing  returns 1 for success, 0 for failure
	...
	// FormatNumber formats the value of the GString0 using the format
	...
	GString0 & operator=(char);
	GString0 & operator=(unsigned __int64);
	GString0 & operator=(__int64);
	GString0 & operator=(const char *);
	GString0 & operator=(const signed char *);
	GString0 & operator=(unsigned char);
	GString0 & operator=(signed char);
	GString0 & operator=(short);
	GString0 & operator=(unsigned short);
	GString0 & operator=(int);
	GString0 & operator=(unsigned int);
	GString0 & operator=(long);
	GString0 & operator=(unsigned long);
	GString0 & operator=(float);
	GString0 & operator=(double);
	GString0 & operator=(long double);
	GString0 & operator=(const GString &);
	GString0 & operator=(const GString32 &);
	GString0 & operator=(const GString0 &);

	// append operators, enables a GString0 to mimic an ostream for simple porting to faster GString0s
	GString0 & operator<<(unsigned __int64);
	GString0 & operator<<(__int64);
	GString0 & operator<<(const char *);
	GString0 & operator<<(const signed char *);
	...
	friend GString operator+(GString0 &, GString0 &);
	friend GString operator+(GString0 &, const char *);
	friend GString operator+(const char *, GString0 &);
	friend GString operator+(GString0 &, const signed char *);
	friend GString operator+(const signed char *, GString0 &);
	friend GString operator+(GString0 &, char);
	friend GString operator+(char, GString0 &);
	friend GString operator+(GString0 &, unsigned char);
	friend GString operator+(unsigned char, GString0 &);
	friend GString operator+(GString0 &, signed char);
	friend GString operator+(signed char, GString0 &);
