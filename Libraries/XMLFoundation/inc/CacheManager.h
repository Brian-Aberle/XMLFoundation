...
public:
	...
	// getObject() should only be called by the XMLObject factory - not the application layer
	...
	// in the cache then returned. This allows access to an object when the data type is unknown. 
	XMLObject *getObject(const char *pzXMLTag, const char* oid, const char *pzUpdateTime, int *bDidCreate = 0);
	void enterObject(const char *oid, const char *pzClassName, XMLObject *pO );

	// [pzOIDClassName] is a string concatination of OID and the C++ Class Name of the object derived from XMLObject
	// [pzTagName] and an XMLElement or XMLAttribute name registered with MapMember
	// returns 1 on success, 0 if pzOIDClassName object instance was not found in the cache
	int getMappedValue(const char *pzOIDClassName, const char* pzTagName, GString *strValueDest, int nSource);
	int setMappedValue(const char* pzOIDClassName, const char* pzTagName, const char *pzValue);
	...
	// is not incremented and therefore the scope of duration is undefined.  If you hold onto the 
	// return value beyond your stack frame, calling IncRef() would make the pointer safe from going out of scope.
	// argument [hashKey] is the string concatination of the OID and C++ class name that was derived from XMLObject
	...
	// ****************************************************************
	void EnterCacheData(__int64 length, void *pData, const char *pzKey);
	void *GetCacheData(const char *pzKey, __int64 *length = 0 );
