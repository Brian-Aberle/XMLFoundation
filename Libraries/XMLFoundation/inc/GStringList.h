...
public:
	...
	// rslt = lst.Serialize("---", 2, 5);// "C---D---E---F---G"
	const char *Serialize(const char *pzDelimiter, int nStartIndex = 0, __int64 nItemCount = -1);
	// same as Serialize, but you supply the destination, that you may have PreAlloc()'d so it will be faster.
	void SerializeTo(GString &strDest, const char *pzDelimiter, int nStartIndex = 0, __int64 nItemCount = -1);
	...
	int ToFileAppend(const char *pzDelimiter, const char *pzPathAndFileName, int nStartIndex = 0, __int64 nItemCount = -1);
