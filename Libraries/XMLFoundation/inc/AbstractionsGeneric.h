...
{
public:
	void removeAll(userStringList pList)						// added 2024
	{
		GStringList* pTypedList = (GStringList*)pList;
		pTypedList->RemoveAll();
	}
	void remove(userStringList pList, const char* pString, bool bMatchCase)		// added 2024
	{
		GStringList* pTypedList = (GStringList*)pList;
		pTypedList->Remove(pString, bMatchCase, 0); // removes the first or only occurance of [pString] in [pTypedList]
	}
