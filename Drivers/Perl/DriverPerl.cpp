...
// Also used to index the [Perl] section in txml.txt
extern "C" MSFT_EXPORT char *GetDriverName(void)
{
	static char pzInterface[] = "Perl";
	return pzInterface;
}
...
// pzComponentPath = value from txml.txt. "Path" entry for [Perl] section.
// pzComponentSettings = value from txml.txt. "Settings" entry for [Perl] section.  
extern "C" MSFT_EXPORT void *Construct(const char *pzComponentPath, const char *pzComponentSettings)
{
	return new PerlInterface(pzComponentPath,pzComponentSettings);
}
...
{
	PerlInterface *pI = (PerlInterface *)pInst;
	return pI->IsAvailable(pzComponent,pzInterface,pzMethodName);
}

// In the case of perl, return a list of .pl files
...
// In the case of perl, return the methods specified in "ExposedMethods"
...
// In the case of perl, return the details specified in "ExposedMethods"
